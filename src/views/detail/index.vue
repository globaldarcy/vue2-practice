                    ...
                    <div class="choose">
                        <div class="chooseArea">
                            ...
                            <dl v-for="(item, index) in spuSaleAttrList" :key="item.id">
                                ...
                                <dd changepirce="0" :class="{ active: subItem.isChecked != 0 }" v-for="subItem in item.spuSaleAttrValueList" :key="subItem.id" @click="changeItem(subItem, item.spuSaleAttrValueList, index)">{{ subItem.saleAttrValueName }}</dd>
                            </dl>
                        </div>
                        <div class="cartWrap">
                            <div class="controls">
                                <input autocomplete="off" type="text" class="itxt" v-model.number="number" @change="changeNumber($event)" />
                                <a href="javascript:" class="plus" @click="number++">+</a>
                                <a href="javascript:" class="mins" @click="number > 1 ? number-- : (number = 1)" :class="{ disabled: number == 1 }">-</a>
                            </div>
                            <div class="add">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...mapActions('detail', ['getGoodsInfo', 'getUpdateShopCart']),
            changeItem(item, list, index) {
                list.forEach((element) => {
                    element.isChecked = 0;
                });
                item.isChecked = 1;
                this.skuInfo.skuSaleAttrValueList[index].saleAttrValueName = item.saleAttrValueName;
            },
            changeNumber(e) {
        ...
        mounted() {
            this.$store.dispatch('detail/getGoodsInfo', this.$route.params.productId);
            // this.getGoodsInfo(this.$route.params.productId);
        },
    };
</script>
