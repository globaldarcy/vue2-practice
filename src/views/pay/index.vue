    ...
    <div class="pay-main">
        <div class="pay-container">
            <div class="checkout-tit">
                ...
                <div class="paymark">
                    <span class="fl">
                        请您在提交订单<em class="orange time">4小时</em>之内完成支付，超时订单会自动取消。订单号：<em>{{ this.orderId }}</em>
                    </span>
                    <span class="fr">
                ...
                try {
                    ...
                    if (!this.timer) {
                        this.timer = setInterval(async () => {
                            let status = await this.$api.reqPayStatus(this.orderId);
                            console.log(status);
                            if (status.code == 200) {
                                clearInterval(this.timer);
                                this.payCode = status.code;
                                this.$msgbox.close();
                                this.$router.push('/pay-success');
                            }
                        }, 1000);
